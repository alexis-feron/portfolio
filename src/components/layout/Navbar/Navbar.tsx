...
export const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const isOverlap = (
      rect1: { top: number; bottom: number },
      rect2: { top: number; bottom: number },
      scrollY: number
    ) => {
  ...
  return (
    <header
      className={classes(
        'flex flex-col items-center justify-start p-0 w-2xl fixed top-outer left-outer bottom-outer z-40 isolate',
    ...
    >
      ...
      >
        <img
          ...
          alt="Alexis Feron"
          className="w-11.25 h-11.25 min-w-11.25 min-h-11.25 rounded-full object-cover hover:animate-[pulse-blue_2s_infinite]"
        />
      </RouterLink>
        ...
        <div className="-rotate-90 flex flex-row-reverse gap-m whitespace-nowrap max-h-[800px]:gap-4 max-h-[800px]:text-sm">
          {navLinks.map(({ label, pathname }) => (
            <RouterLink
              ...
              key={label}
              className={classes(
                'p-m text-[rgb(var(--rgbText)/0.8)] font-medium text-(length:--headerNavFontSize) no-underline relative transition-[color] duration-s ease-linear delay-100 leading-none inline-flex items-center',
                'hover:text-(--colorTextBody) active:text-(--colorTextBody) focus:text-(--colorTextBody) aria-[current=page]:text-(--colorTextBody)',

                // After element for underline
                'after:content-[""] after:absolute after:right-s after:left-s after:h-1 after:bg-accent after:scale-x-0 after:origin-right',
      ...
      <NavbarIcons desktop className={classes('mb-6', 'nav-mobile-hidden')} />
      <nav
        className={classes(
          'fixed inset-0 bg-[rgb(var(--rgbBackground)/0.9)] transition-colors duration-l ease-fast-out-slow-in flex flex-col items-center justify-center backdrop-blur-lg z-100',
      ...
      >
        ...
        <RouterLink
          ...
          scroll={false}
          className={classes(
            'absolute top-outer left-outer z-101',
            'flex p-s rounded-full', // Simplified to uniform padding
            'pb-s' // Explicit bottom padding to match
          )}
          aria-label="Home"
          onClick={handleMobileNavClick}
        >
          <div className="relative grid grid-cols-[100%] grid-rows-[100%] overflow-hidden w-11.25 h-11.25 rounded-full isolate">
        ...
        {navLinks.map(({ label, pathname }, index) => (
          <RouterLink
            ...
            key={label}
            className={classes(
              'w-full text-(length:--headerNavFontSize) text-center no-underline text-(--colorTextBody) p-l transition-opacity duration-s ease-fast-out-slow-in relative -top-m',
              'motion-safe:transition-[transform,opacity] motion-safe:-translate-y-xl',
              'max-[820px]:max-h-[420px]:top-auto',
              'max-[400px]:[--headerNavFontSize:calc((18/16)*1rem)] max-h-[360px]:[--headerNavFontSize:calc((18/16)*1rem)]',

              // After element
              'after:content-[""] after:absolute after:top-1/2 after:right-3xl after:left-3xl after:h-1 after:bg-accent after:scale-x-0 after:-translate-y-px after:origin-right',
              'motion-safe:after:transition-transform motion-safe:after:duration-m motion-safe:after:ease-fast-out-slow-in',
              'hover:after:scale-x-100 hover:after:origin-left',
              'active:after:scale-x-100 active:after:origin-left',
              'focus:after:scale-x-100 focus:after:origin-left',
  ...
  >
    {socialLinks.map(({ label, url, icon }) => (
      <a
        ...
        className={classes(
          'flex items-center justify-center p-s w-2xl h-2xl text-(--colorTextLight) transition-[color] duration-m ease-fast-out-slow-in',
      ...
      >
        <Icon
          className="fill-current transition-[fill] duration-m ease-linear"
          icon={icon}
        />
      </a>
    ))}
