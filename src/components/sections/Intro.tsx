...
export function Intro({ id, sectionRef, scrollIndicatorHidden, ...rest }: IntroProps) {
  ...
  return (
    <Section
      className="h-screen flex items-center justify-center flex-col outline-none focus:outline-none"
      as="section"
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      <Transition in key={theme.themeId} timeout={1000}>
        {(visible: boolean, status: string) => (
          <Fragment>
            ...
            <header
              className={classes(
                'max-w-256 w-full relative -top-l',
                'max-[2080px]:max-w-230',
                'max-[1680px]:max-w-195',
                'max-[696px]:-top-3xl',
                'max-[400px]:-top-xl',
            ...
            >
              <h1
                className={classes(
                  'uppercase text-[calc((24/16)*1rem)] tracking-[0.3em] text-(--colorTextBody) mb-2xl mt-0 font-medium leading-none opacity-0 transition-opacity duration-l ease-fast-out-slow-in delay-200',
              ...
              >
                <VisuallyHidden className="sr-only">{`Web Developer`}</VisuallyHidden>
                <span
                  aria-hidden
                  className={classes(
                ...
                >
                  <span
                    className={classes(
                      ...
                      status === 'entering' &&
                        'animate-[introTextReveal_1.5s_var(--ease-fast-out-slow-in)_var(--delay)_forwards]',
                      status === 'entering' &&
                        'motion-safe:after:animate-[reveal_1.5s_var(--ease-fast-out-slow-in)_var(--delay)_forwards]',

                      // data-status entered
                      status === 'entered' && 'text-(--colorTextTitle)',
                      status === 'entered' &&
                        'after:opacity-100 after:scale-x-0 after:origin-right',

                      // data-status exiting
                      status === 'exiting' &&
                        'text-(--colorTextTitle) opacity-0 absolute top-0 z-0'
                    )}
                    data-status={status}
                  ...
                  <span
                    className={classes(
                      'content-[""] h-0.5 bg-[rgb(var(--rgbText)/0.3)] w-[120%] flex ml-5 animate-[0.8s_var(--ease-fast-out-slow-in)_1s_forwards] origin-left opacity-0 relative top-[0.05em]',
                      status === 'entering' &&
                        'animate-[fadeIn_0.8s_var(--ease-fast-out-slow-in)_1s_forwards]',
                      status === 'entering' &&
                        'motion-safe:animate-[introLine_0.8s_var(--ease-fast-out-slow-in)_1s_forwards]',
            ...
            <RouterLink
              href="/#details"
              className={classes(
                'border-2 border-[rgb(var(--rgbText)/0.4)] rounded-[20px] w-6.5 h-9.5 fixed bottom-16 transition-opacity duration-l ease-linear opacity-0',

                // Before element (scroller)
                'before:content-[""] before:h-1.75 before:w-0.5 before:bg-[rgb(var(--rgbText)/0.4)] before:rounded-sm before:absolute before:top-1.5 before:left-1/2 before:-translate-x-px',
                ...
                scrollIndicatorHidden && 'opacity-0! motion-safe:translate-y-5',
            ...
            <RouterLink
              href="/#details"
              className={classes(
                'fixed bottom-10 opacity-0 animate-[1.5s_infinite] transition-opacity duration-m cubic-bezier-[0.8,0.1,0.27,1] p-5',
                'motion-safe:animate-[introMobileScrollIndicator_1.5s_infinite] motion-safe:transition-[opacity,transform] motion-safe:translate-y-5',
